<template>
                    ...
                    <tbody class="">
                        <tr class="border-t border-l border-r border-blue-600">
                            ...
                            <td class="bg-tbody relative px-6 py-2">
                                ...
                                <div v-if="isAction" class="actionMenu absolute top-auto right-auto w-20 shadow-md rounded-md z-50">
                                    <div class="flex flex-col">
                                        <button @click="toggleFileLeave" class="profile-setLink hover:bg-blue-100 cursor-pointer flex items-center justify-center gap-x-2 p-2 border-b border-blue-600">
                                        ...
                                        <button  class="profile-setLink hover:bg-blue-100 cursor-pointer flex items-center justify-center gap-x-2 p-2">
    ...
    <AdminModal @close="toggleFileLeave" :modalAdmin="modalFileLeave">
        <div class="modal-content">
                <div class="modal-header lg:px-5 px-3 py-2">
                    <h1 class="text-lg capitalize font-bold lg:text-xl">File Leave</h1>
                </div>
                <form action="" class="px-3 pt-5 gap-y-4 flex flex-col lg:px-5">
                    <div class="flex flex-col gap-y-1.5">
                        <p class="text-base font-semibold">Employee Name: </p>
                        <AdminInput v-model="employeeName" type="text" placeholder="Full Name"/>
                    </div>
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 ">
                        <div class="flex flex-col gap-y-1.5">
                            <p class="text-base font-semibold">Type of Leave:</p>
                            <AdminInput v-model="leaveType" type="text" placeholder="Offset"/>
                        </div>
                        <div class="flex flex-col gap-y-1.5">
                            <p class="text-base font-semibold">Date</p>
                            <AdminInput v-model="date" type="date"/>
                        </div>
                    </div>
                    <div class="flex flex-col gap-y-1.5">
                        <p class="text-base font-semibold">Reason</p>
                        <div class="reasonField w-full rounded-md px-2 py-1">
                            <textarea class="resize-none bg-transparent border-none outline-none text-base w-full lg:h-32 xl:h-40" placeholder="reason"></textarea>
                        </div>
                    </div>
                </form>
            </div>
...
const modalFileLeave = ref(null);

const toggleFileLeave = ()=>{
    modalFileLeave.value = !modalFileLeave.value
}
</script>
